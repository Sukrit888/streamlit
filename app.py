...
def classify_lcvs_equally(lcv_ids, seed=None):
    ...
    # Get unique, non-null LCVs
    unique_lcvs = [lcv for lcv in lcv_ids.unique() if pd.notna(lcv)]
    
    # Use a specific seed for the random shuffle to make it consistent per date
    # This ensures the allocation is the same for a given date, but different for different dates.
    if seed is not None:
        random.seed(seed)
...
if df is not None:
    ...
    # Use the date to create a consistent seed for the random classification
