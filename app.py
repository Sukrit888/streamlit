...
if df is not None:
    ...
    # Changed from multiselect to selectbox for MGS
    unique_mgs = df_filtered_date['MGS'].unique()
    selected_mgs = st.sidebar.selectbox('Select MGS', options=unique_mgs)

    # Adjusted filtering logic for single selectbox
    df_filtered_mgs = df_filtered_date[df_filtered_date['MGS'] == selected_mgs]
